...
void Corpus::init(bool logFreqSetting, float rmsMinCutoffSetting, 
...
void Corpus::beginAnalyse(const vector<ofFile>& fileList, bool logFreqSetting, 
...
void Corpus::draw(ofCamera& camera, glm::vec3 mouse, bool drawPointsEnabled, bool listingInProgress, bool pointPickEnabled, bool pointPickSelected) {
...
void Corpus::deinterleaveAudioData(vector<vector<float>>& deinterleavedData, float* interleavedData, 
...
float Corpus::spectralCentroidOneFrame(float* input, float sampleRate, bool logFreq) {
...
int Corpus::pointPicker(glm::vec3 mouse, ofCamera camera, bool selected) {
...
void Corpus::soundController(bool pointPickEnabled) {
...
glm::vec3 Corpus::getMaxDimensions() {
	return { centroidMax, rmsMax, timeMax };
}

int Corpus::getPointCount() {
	return points.getNumVertices();
}

int Corpus::getFileFromPoint(int index) {
